...
const form = useForm({
    transaction_date: props.cashFlow ? (props.cashFlow.transaction_date || '').substring(0,10) : new Date().toISOString().substr(0,10),
    reference_number: props.cashFlow ? props.cashFlow.reference_number : '',
    description: props.cashFlow ? props.cashFlow.description : '',
    attachment: null,
    transactions: props.cashFlow && props.cashFlow.transactions && props.cashFlow.transactions.length > 0
        ? props.cashFlow.transactions.map(t => newDetail(t))
        : [ newDetail() ]
});
...
<template>
    <Head title="Edit Cash Flow" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Cash Flow</h2>
                    ...
                    <div class="p-6">
                        <form @submit.prevent="submit">
                            ...
                            <div class="flex items-center justify-end mt-8">
                                ...
                                <PrimaryButton class="ml-4" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">Update Cash Flow</PrimaryButton>
                            </div>
                        </form>
                    </div>
                </div>
