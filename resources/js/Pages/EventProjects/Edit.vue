...
const initialDetails = (props.eventProject.details || []).map(d => ({
    budget_item_id: d.budget_item_id,
    allocated_amount: d.allocated_amount,
    approved_amount: d.approved_amount
}));
...
const form = useForm({
    event_name: props.eventProject.event_name || '',
    event_date: props.eventProject.event_date ? props.eventProject.event_date.substr(0,10) : new Date().toISOString().substr(0,10),
    location: props.eventProject.location || '',
    description: props.eventProject.description || '',
    note: props.eventProject.note || '',
    status: props.eventProject.status || 'planned',
    attachment: null,
    cash_flow_id: props.eventProject.cash_flow_id || null,
...
<template>
    <Head title="Edit Event Project" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Event Project</h2>
